...
const allPages = (
  ...
  return {
    ...
    list: renderList({ items: [item], total: 1 }),
    search: renderSearch({
      q: "investment",
      items: [item],
      limit: 24,
...
describe("design system", () => {
  test("inlines exactly one stylesheet and fetches nothing", () => {
    for (const [name, html] of Object.entries(allPages())) {
      ...
      // No origin of any kind: no font, image, stylesheet or script host.
  ...
  test("stays inside the inline-style budget", () => {
    expect(Buffer.byteLength(PUBLIC_STYLES)).toBeLessThanOrEqual(12 * 1024);
  });
});
...
describe("accessibility", () => {
  ...
  test("marks the current navigation entry exactly once", () => {
    const market = renderList({ items: [], total: 0 });
    const search = renderSearch({ q: "", items: [], limit: 24, offset: 0 });
    // The stylesheet also mentions the attribute, so anchor on the markup.
    expect(market.match(/aria-current="page">/g)?.length).toBe(1);
    expect(market).toContain(
      '<a href="/market" aria-current="page">能力目录</a>',
    );
    expect(search.match(/aria-current="page">/g)?.length).toBe(1);
    expect(search).toContain('<a href="/search" aria-current="page">搜索</a>');
    // The constant pages belong to no section, so they mark nothing.
    expect(renderNotFound().match(/aria-current="page">/g)).toBeNull();
    expect(renderUnavailable().match(/aria-current="page">/g)).toBeNull();
  });

  test("labels the search field and pairs it with a visible submit control", () => {
  ...
  test("hides decoration from assistive technology", () => {
    ...
    expect(html).toContain('class="tone tone-');
    for (const tag of tagsOf(html).filter((tag) => tag.includes('class="tone')))
...
describe("honesty", () => {
  ...
  test("the catalogue page prints the real total inside the page grid", () => {
    const many = Array.from({ length: 3 }, (_, index) =>
      summaryOf({ slug: `item-${index}`, packageName: `item-${index}` }),
    );
    const complete = renderList({ items: many, total: 3 });
    expect(complete).toContain('<div class="wrap">\n<h1>能力目录</h1>');
    expect(complete).toContain("共 3 个公开条目");
    // A cached page holds fewer entries than the catalogue: it says so.
    const truncated = renderList({ items: many, total: 42 });
    expect(truncated).toContain("共 42 个公开条目");
    expect(truncated).toContain("本页列出 3 条");
  });
...
describe("capability rig", () => {
  ...
  test("draws both sides when both declared members", () => {
    ...
    expect(rig).toContain("<strong>3</strong> 个连接器");
  ...
  test("omits the side that declared nothing", () => {
    ...
    expect(serversOnly).toContain("个连接器");
  ...
  test("never prints a zero", () => {
    for (const [agents, servers] of [
      ...
      expect(rig).not.toContain("0 个连接器");
    }
  });
});
...
describe("empty states", () => {
  test("distinguishes an empty catalogue from an empty search", () => {
    expect(renderList({ items: [], total: 0 })).toContain(
      "目录中还没有公开条目。",
    );
    expect(
