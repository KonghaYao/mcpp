...
export function ResourceForAgents(
  ...
  if (
    organizationPrefix !== undefined &&
    !isValidAgentOrganizationPrefix(organizationPrefix)
  ) {
    throw new Error("MCPP agent resources require a valid organizationPrefix");
  }
  if (cacheScope === "private" && !authorizationContext) {
    throw new Error(
      "MCPP private Agent resource cache requires an opaque authorization context",
    );
  }
  ...
  server.registerResource(
    ...
    new ResourceTemplate(template, {
      list: async () => {
        ...
        const cached = cache?.get<Awaited<ReturnType<typeof scanAgentsDir>>>(
          key,
          { cacheVersion },
        );
        const agents = cached ?? (await scanAgentsDir(agentsDir, scanOptions));
        if (!cached)
          cache?.set(key, agents, { scope: cacheScope, ttlMs, cacheVersion });
        return {
    ...
    }),
    {
      title: "MCP Agent",
      description:
        "An untrusted subagent configuration that requires host approval before activation",
    },
    async (uri, variables) => {
      const rawName = variables.agentName;
      const agentName = Array.isArray(rawName)
        ? (rawName[0] ?? "")
        : (rawName ?? "");
      ...
      const cached = cache?.get<Awaited<ReturnType<typeof readAgentResource>>>(
        key,
        { cacheVersion },
      );
      const resource =
        cached ?? (await readAgentResource(agentsDir, agentName, scanOptions));
      if (!cached && resource) {
      ...
      if (!resource || resource.uri !== uri.href) {
        throw new ResourceNotFoundError(
          uri.href,
          `Agent resource '${uri.href}' not found`,
        );
      }
