...
import { agentUri, isValidAgentName, parseAgentFrontmatter } from "./agent.ts";
...
async function request(
  ...
  const response = await gateway.fetch(
    new Request("http://localhost/agents/mcp", {
      method: "POST",
      headers: {
...
async function createFixture(): Promise<string> {
  ...
  await Promise.all([
    ...
    writeFile(
      join(root, "missing-description", "agent.md"),
...
describe("MCP Agent Resource 投影", () => {
  ...
  test("只扫描名称一致且有效的 agent.md", async () => {
    ...
    try {
      const agents = await scanAgentsDir(root, {
        organizationPrefix: "peri.example",
      });
      expect(
        agents.map(({ name, uri, description }) => ({
          name,
          uri,
          description,
        })),
  ...
  test("拒绝符号链接入口与超出大小限制的配置", async () => {
    ...
    try {
      ...
      await symlink(
        join(root, "code-reviewer", "agent.md"),
        join(root, "linked", "agent.md"),
      );
      expect(await readAgentResource(root, "linked")).toBeUndefined();
      expect(
        await readAgentResource(root, "code-reviewer", { maxAgentBytes: 32 }),
      ).toBeUndefined();
    } finally {
      await rm(root, { recursive: true, force: true });
    }
