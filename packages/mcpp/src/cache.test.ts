...
describe("McppCache", () => {
  ...
  test("private entry 按 authorization context 隔离，且通知可失效", () => {
    ...
    expect(cache.get<string>({ ...base, authorizationContext: "user-a" })).toBe(
      "a",
    );
    expect(cache.get<string>({ ...base, authorizationContext: "user-b" })).toBe(
      "b",
    );
    expect(() =>
      cache.set<string>(base, "missing-context", { scope: "private" }),
    ).toThrow();

    cache.markResourceStale("server-a", "private://doc");
    expect(
      cache.get<string>({ ...base, authorizationContext: "user-a" }),
    ).toBeUndefined();
    expect(
      cache.get<string>(
  ...
  test("Server Cache Version 相等时命中，不相等时拒绝旧条目", () => {
    ...
    expect(
      cache.get<string[]>(listKey, { cacheVersion: "snapshot-v1" }),
    ).toEqual(["demo"]);
    expect(cache.get<string>(contentKey, { cacheVersion: "snapshot-v1" })).toBe(
      "content-v1",
    );
    expect(cache.get(listKey, { cacheVersion: "snapshot-v2" })).toBeUndefined();
    expect(
      cache.get(contentKey, { cacheVersion: "snapshot-v2", allowStale: true }),
    ).toBeUndefined();
  });
  ...
  test("Server Cache Version 相等时可命中 TTL 已过期但未 stale 的条目", () => {
    ...
    now += 100;
    expect(cache.get(key)).toBeUndefined();
    expect(cache.get<string[]>(key, { cacheVersion: "snapshot-v1" })).toEqual([
      "demo",
    ]);
