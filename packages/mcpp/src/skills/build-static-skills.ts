...
export async function buildStaticSkillResources(
  ...
  for (const file of files) {
    const content = await readSkillResourceFile(
      skillsDir,
      file.skillName,
      file.relativePath,
      options,
    ...
    if (!content) {
      throw new Error(
        `MCPP static skill build: '${file.uri}' changed or failed validation while bundling`,
      );
    }
    resources.push({
      ...file,
...
export function renderStaticSkillResourcesModule(
  ...
  if (!/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(exportName)) {
    throw new Error(
      "MCPP static skill build: exportName must be a TypeScript identifier",
    );
  }
  return [
