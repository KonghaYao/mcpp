...
function hasMatchingContent(resource: StaticSkillResourceFile): boolean {
  if (resource.contentKind === "text") {
    return (
      ...
      utf8Size(resource.text) === resource.size
    );
  }
  return (
    typeof resource.blob === "string" &&
    resource.text === undefined &&
    blobSize(resource.blob) === resource.size
  );
}

function compareResources(
  left: StaticSkillResourceFile,
  right: StaticSkillResourceFile,
): number {
...
export function createStaticSkillResources(
  ...
  for (const resource of resources) {
    ...
    if (
      ...
      classifySkillResourceFile(relativePath, resource.contentKind)
        .contentKind !== resource.contentKind ||
      classifySkillResourceFile(relativePath, resource.contentKind).mimeType !==
        resource.mimeType ||
    ...
    if (
      ...
      (expectedKind === "skill" && resource.contentKind !== "text")
    )
      continue;
    candidates.set(resourceKey(resource.skillName, relativePath), {
      ...resource,
      relativePath,
    });
  }
  ...
  for (const skillName of [...candidatesBySkill.keys()].sort((left, right) =>
    ...
    const root = skillFiles.find(
      (resource) => resource.relativePath === "SKILL.md",
    );
    if (
      !root ||
      root.size > limits.maxFileBytes ||
      root.size > limits.maxSkillBytes ||
      totalBytes + root.size > limits.maxTotalBytes
    ) {
      continue;
    }
...
function indexStaticResources(
  ...
  return {
    ...
    byKey: new Map(
      files.map((resource) => [
        resourceKey(resource.skillName, resource.relativePath),
        resource,
      ]),
    ),
  };
...
function resourceDescription(
  namePrefix: string,
  resource: StaticSkillResourceFile,
): string {
  if (resource.kind === "file")
    return resource.description ?? `Skill file: ${resource.relativePath}`;
...
export function ResourceForStaticSkills(
  ...
  if (cacheScope === "private" && !authorizationContext) {
    throw new Error(
      "MCPP private static Skill resource cache requires an opaque authorization context",
    );
  }
  ...
  server.registerResource(
    ...
    new ResourceTemplate("skill://{skillName}/{+path}", {
      list: () => {
        const key = origin
          ...
          : undefined;
        const cached = key
          ? cache?.get<StaticSkillListResponse>(key, { cacheVersion })
          : undefined;
        ...
        if (key)
          cache?.set(key, response, { scope: cacheScope, ttlMs, cacheVersion });
        return response;
      },
    }),
    {
    ...
    async (uri, variables) => {
      ...
      const relativePath = decodeSkillFilePath(
        firstTemplateVar(variables.path),
      );
      ...
      if (!resource) {
        throw new ResourceNotFoundError(
          uri.href,
          `Skill resource '${uri.href}' not found`,
        );
      }
      const key = origin
        ? cacheKey("resources/read", { uri: uri.href })
        : undefined;
      const cached = key
        ? cache?.get<StaticSkillReadResponse>(key, { cacheVersion })
        : undefined;
      if (cached) return cached;
      const contents =
