...
import { decodeSkillFilePath, skillFileUri } from "./skill-uri.ts";
...
async function createFixture(): Promise<string> {
  ...
  await Promise.all([
    ...
    writeFile(
      join(skillRoot, "automation", "nested", "check.py"),
      "print('checked')\n",
    ),
    writeFile(
      join(skillRoot, "automation", "nested", "instructions.custom"),
      "custom instructions\n",
    ),
    writeFile(
      join(skillRoot, "automation", "nested", "payload.data"),
      Buffer.from([0, 1, 2]),
    ),
    writeFile(
      join(skillRoot, "private", "notes.md"),
  ...
  await symlink(
    join(root, "outside"),
    join(skillRoot, "references", "outside-link"),
  );
...
describe("Skill 目录附属 Resource 投影", () => {
  test("递归枚举 Skill 根内的普通文件，拒绝隐藏路径与符号链接", async () => {
    ...
    try {
      ...
      expect(files.find((file) => file.relativePath === "SKILL.md")?.kind).toBe(
        "skill",
      );
      expect(
        files.find((file) => file.relativePath === "automation/nested/check.py")
          ?.mimeType,
      ).toBe("text/x-python");
      expect(
        files.find(
          (file) =>
            file.relativePath === "automation/nested/instructions.custom",
        ),
      ).toMatchObject({
        mimeType: "text/plain",
        contentKind: "text",
      ...
      expect(
        files.find((file) => file.relativePath === "fake-text.md"),
      ).toMatchObject({
  ...
  test("读取任意目录与未知扩展名的附属文件，且不允许绕过 Skill 根", async () => {
    const root = await createFixture();
    try {
      const reference = await readSkillResourceFile(
        root,
        "demo",
        "references/中文 文档.md",
      );
      ...
      const script = await readSkillResourceFile(
        root,
        "demo",
        "automation/nested/check.py",
      );
      expect(script).toMatchObject({
        mimeType: "text/x-python",
        text: "print('checked')\n",
      });
      const binary = await readSkillResourceFile(
        root,
        "demo",
        "automation/nested/payload.data",
      );
      ...
      expect(
        await readSkillResourceFile(root, "demo", "references/outside-link"),
      ).toBeUndefined();
      expect(
        decodeSkillFilePath("references/%2E%2E/secret.md"),
      ).toBeUndefined();
  ...
  test("普通目录默认可公开", async () => {
    ...
    try {
      expect(
        (await scanSkillResourceFiles(root)).some(
          (file) => file.relativePath === "private/notes.md",
        ),
      ).toBe(true);
    } finally {
      await rm(root, { recursive: true, force: true });
  ...
  test("拒绝被符号链接替换的 Skill 根或子目录", async () => {
    ...
    try {
      ...
      expect(
        (await scanSkillResourceFiles(root)).some((file) =>
          file.relativePath.startsWith("references/"),
        ),
      ).toBe(false);
  ...
  test("静态 registry 跨 Server 实例按 TTL 缓存资源列表和读取结果", async () => {
    ...
    const gateway = createGatewayRoutes([
      {
        ...
        createServer: () => {
          const server = new McpServer({
            name: "static-skill-cache-fixture",
            version: "1",
          });
    ...
    try {
      ...
      await request(gateway, "resources/read", {
  ...
  test("构建期 registry 在删除源目录后仍可供 Worker 形态读取", async () => {
    ...
    try {
      const reference = resources.find(
        (resource) => resource.relativePath === "references/guide.md",
      );
      ...
      expect(
        createStaticSkillResources([
          ...resources,
          {
            ...reference!,
            skillName: "orphan",
            uri: "skill://orphan/references/guide.md",
          },
          {
            ...reference!,
            relativePath: ".hidden.md",
            uri: "skill://demo/.hidden.md",
          },
        ]).some(
          (resource) =>
            resource.skillName === "orphan" ||
            resource.relativePath === ".hidden.md",
        ),
      ).toBe(false);
      expect(renderStaticSkillResourcesModule(resources)).toContain(
      ...
      expect(
        (listed.resources as Array<{ uri: string }>).map(
          (resource) => resource.uri,
        ),
      ).toContain("skill://demo/references/guide.md");
  ...
  test("通过 MCP resources/list 和 resources/read 传递附属文件", async () => {
    ...
    try {
      ...
      const listed = await request(gateway, "resources/list", {});
      const resources = listed.resources as Array<{
        uri: string;
        name: string;
      }>;
      expect(
