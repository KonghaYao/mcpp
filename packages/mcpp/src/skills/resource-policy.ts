...
function positiveInteger(
  ...
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new Error(
      `MCPP skill resources: ${label} must be a positive safe integer`,
    );
  }
  return value;
}
...
export function resolveSkillResourceOptions(
  ...
  return {
    limits: {
      ...
      maxSkillBytes: positiveInteger(
        options?.maxSkillBytes,
        DEFAULT_LIMITS.maxSkillBytes,
        "maxSkillBytes",
      ),
      maxSkillFiles: positiveInteger(
        options?.maxSkillFiles,
        DEFAULT_LIMITS.maxSkillFiles,
        "maxSkillFiles",
      ),
      maxTotalFiles: positiveInteger(
        options?.maxTotalFiles,
        DEFAULT_LIMITS.maxTotalFiles,
        "maxTotalFiles",
      ),
