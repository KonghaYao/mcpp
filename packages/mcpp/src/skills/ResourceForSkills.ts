...
import { decodeSkillFilePath, firstTemplateVar } from "./skill-uri.ts";
import {
...
export function ResourceForSkills(
  ...
  if (cacheScope === "private" && !authorizationContext) {
    throw new Error(
      "MCPP private Skill resource cache requires an opaque authorization context",
    );
  }
  const descriptionFor = (name: string, d?: string) =>
    d ?? `${namePrefix}:${name}`;
  ...
  server.registerResource(
    ...
    new ResourceTemplate("skill://{skillName}/{+path}", {
      list: async () => {
        const key = cacheKey("resources/templates/list", {
          template: "skill://{skillName}/{+path}",
        });
        const cached = cache?.get<
          Awaited<ReturnType<typeof scanSkillResourceFiles>>
        >(key, { cacheVersion });
        const files =
          cached ?? (await scanSkillResourceFiles(skillsDir, resourceLimits));
        if (!cached)
          cache?.set(key, files, { scope: cacheScope, ttlMs, cacheVersion });
        return {
    ...
    async (u, variables) => {
      ...
      const cached = cache?.get<
        Awaited<ReturnType<typeof readSkillResourceFile>>
      >(key, { cacheVersion });
      const resource =
        cached ??
        (await readSkillResourceFile(
          skillsDir,
          name,
          relativePath,
          resourceLimits,
      ...
      if (!resource) {
        throw new ResourceNotFoundError(
          u.href,
          `Skill resource '${u.href}' not found`,
        );
      }
