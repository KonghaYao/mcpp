...
async function contentKindForFile(
  path: string,
): Promise<SkillResourceContentKind | undefined> {
  try {
...
export async function scanSkillResourceFiles(
  ...
  for (const skill of skills) {
    ...
    const entry = candidates.find(
      (candidate) => candidate.relativePath === "SKILL.md",
    );
    const entryContentKind = entry
      ? await contentKindForFile(entry.absolutePath)
      : undefined;
    if (
      !entry ||
