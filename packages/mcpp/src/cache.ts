...
function keyFor(key: McppCacheKey, scope: McppCacheScope): string {
  if (!key.origin || !key.method)
    throw new Error("MCPP cache key requires origin and method");
  if (scope === "private" && !key.authorizationContext) {
    throw new Error(
      "MCPP private cache entries require an opaque authorization context",
    );
  }
  return stableSerialize({
    ...
    authorizationContext:
      scope === "private" ? key.authorizationContext : undefined,
  });
}
...
export class McppCache {
  ...
  public get<T>(
    ...
    const versionMatches =
      options.cacheVersion !== undefined &&
      entry.cacheVersion === options.cacheVersion;
    if (options.cacheVersion !== undefined && !versionMatches) return undefined;
    const expired =
      entry.expiresAt !== undefined && this.now() >= entry.expiresAt;
    if ((entry.stale || (expired && !versionMatches)) && !options.allowStale)
      return undefined;
  ...
  public markOriginStale(origin: string, methods?: readonly string[]): void {
    for (const entry of this.entries.values()) {
      if (
        entry.origin === origin &&
        (!methods || methods.includes(entry.method))
      )
        entry.stale = true;
    }
  }
  ...
  public markResourceStale(origin: string, resourceUri: string): void {
    for (const entry of this.entries.values()) {
      if (entry.origin === origin && entry.resourceUri === resourceUri)
        entry.stale = true;
    }
  }
