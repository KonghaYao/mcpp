...
function canonicalCacheVersionValue(
  value: unknown,
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new Error("MCPP cache version snapshot requires finite numbers");
    return JSON.stringify(value);
  }
  if (typeof value !== "object") {
    throw new Error(
      "MCPP cache version snapshot must contain only JSON-compatible values",
    );
  }
  if (ancestors.has(value))
    throw new Error("MCPP cache version snapshot must not contain cycles");
  ancestors.add(value);
  try {
    ...
    return `{${Object.keys(record)
      ...
      .map((key) => {
        ...
        if (item === undefined) {
          throw new Error(
            "MCPP cache version snapshot must not contain undefined",
          );
        }
 ...
 */
export async function createCacheVersion(
  snapshot: McppCacheVersionSnapshot,
): Promise<string> {
...
export function createMcppServerFactory(
  ...
  if (!Number.isFinite(ttlMs) || ttlMs < 0) {
    throw new Error(
      "MCPP server cache ttlMs must be a finite non-negative number",
    );
  }
  ...
  return (context) => {
    ...
    if (cacheScope === "private" && !authorizationContext) {
      throw new Error(
        "MCPP private server cache requires an opaque authorization context",
      );
    }
