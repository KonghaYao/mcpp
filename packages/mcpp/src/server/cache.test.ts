...
describe("createCacheVersion", () => {
  ...
  test("拒绝无法跨运行时稳定序列化的值", async () => {
    await expect(
      createCacheVersion({ tools: { value: undefined } as never }),
    ).rejects.toThrow("undefined");
    await expect(createCacheVersion({ tools: Number.NaN })).rejects.toThrow(
      "finite numbers",
    );
    await expect(
      createCacheVersion({ tools: new Date() as never }),
...
describe("createMcppServerFactory", () => {
  test("默认提供 Server Cache Version、共享 MCPP Cache 和一天 TTL", async () => {
    const contexts: Parameters<
      Parameters<typeof createMcppServerFactory>[1]
    >[1][] = [];
    ...
    if (!first || !second)
      throw new Error("Expected two MCPP server cache contexts");
    ...
    expect(
      first.capabilities.extensions?.[MCPP_SERVER_CACHE_VERSION_EXTENSION],
    ).toEqual({
      cacheVersion: "snapshot-v1",
    });
  });
  ...
  test("允许覆盖 TTL，并强制 private cache 提供 authorization context", async () => {
    ...
    expect(() => invalidFactory(context)).toThrow(
    ...
    const factory = createMcppServerFactory(
      ...
      (_request, mcpp) => {
        ...
        expect(mcpp.resourceCache.authorizationContext).toBe(
          "principal-opaque-id",
        );
        return server;
      },
