...
export const MCPP_SERVER_CACHE_VERSION_EXTENSION =
 ...
 */
export function serverCacheVersionCapability(
  cacheVersion: string,
): McppServerCacheVersionCapability {
  if (!cacheVersion.trim())
    throw new Error("MCPP server cacheVersion must be non-empty");
  return { cacheVersion };
}
