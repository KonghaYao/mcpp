...
import { createCatalogPageHandler, renderCatalogPage } from "./catalog-page.ts";
...
describe("Catalog 页面 helper", () => {
  ...
  test("只在配置页面路径响应 GET 与 HEAD", async () => {
    ...
    const head = handle(
      new Request("https://example.test/catalog", { method: "HEAD" }),
    );
    ...
    expect(
      handle(new Request("https://example.test/catalog", { method: "POST" }))
        .status,
    ).toBe(405);
  });
  ...
  test("拒绝不安全的页面与 Catalog 路径", () => {
    expect(() =>
      renderCatalogPage({ catalogPath: "https://other.example/catalog/mcp" }),
    ).toThrow();
    expect(() =>
      renderCatalogPage({ catalogPath: "//other.example/catalog/mcp" }),
    ).toThrow();
    expect(() =>
      createCatalogPageHandler({ pagePath: "/../catalog" }),
    ).toThrow();
  });
});
