...
const listParams = z
  .object({
    ...
    capabilities: z
      .array(z.enum(CATALOG_CAPABILITIES))
      .max(CATALOG_CAPABILITIES.length)
      .optional(),
  })
  .strict();
...
function sortedCapabilities(
  values: readonly CatalogCapability[] | undefined,
): CatalogCapability[] {
...
function validateEntry(
...
): Omit<ResolvedServerCatalogEntry, "entryDigest"> {
  ...
  if (
    ...
    throw new Error(
      `MCPP catalog: server '${entry.id}' requires title, description, and version`,
    );
  }
  if (
    ...
    throw new Error(
      `MCPP catalog: invalid endpoint path '${entry.endpointPath}'`,
    );
  }
...
async function resolveEntries(
...
): Promise<ResolvedServerCatalogEntry[]> {
  ...
  for (const entry of entries) {
    ...
    if (seenPaths.has(value.endpointPath)) {
      throw new Error(
        `MCPP catalog: duplicate endpoint path '${value.endpointPath}'`,
      );
    }
...
): Omit<ResolvedServerCatalogEntry, "endpointPath"> {
...
function matches(
  ...
  if (
    capabilities?.some((capability) => !entry.capabilities.includes(capability))
  )
    return false;
  return true;
}
...
export async function createServerCatalog(
  ...
  server.server.setRequestHandler(
    ...
    { params: getParams },
    (params) => ({
      server: toListEntry(findEntry(resolvedEntries, params.serverId)),
    }),
  );
