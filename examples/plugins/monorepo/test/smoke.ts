...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (demo.status !== 200)
      throw new Error(`Catalog demo 应返回 200，得到 ${demo.status}`);
    ...
    if (
      ...
      !html.includes(
        'cell.classList.add("whitespace-nowrap", "font-medium", "text-slate-950")',
      ) ||
      html.includes("server-card-template")
    ) {
      throw new Error(
        "Catalog demo 未包含 CDN 样式、浅色列表、连接解析、Child MCP 资源表格或复制操作",
      );
    }
    ...
    const browserCatalogBody = (await browserCatalog.json()) as {
      result?: { servers?: unknown[] };
    };
    if (
      browserCatalog.status !== 200 ||
      !browserCatalogBody.result?.servers?.length
    ) {
    ...
    if (
      browserChildDiscover.status !== 200 ||
      !browserChildDiscovery.result?.capabilities?.resources
    ) {
      throw new Error("无 SDK 的浏览器 Child MCP 协商失败，或未声明 Resources");
    }
    if (browserChildDiscovery.result.capabilities.tools) {
      throw new Error(
        "OpenSpec Child MCP 不应在未注册 Tools 时声明 tools capability",
      );
    }
    ...
    const browserResourcesBody = (await browserResources.json()) as {
      result?: {
        resources?: Array<{ uri?: unknown; name?: unknown; title?: unknown }>;
      };
    };
    ...
    if (
      ...
      !browserSkillResources.some(
        (resource) =>
          resource.uri ===
            "skill://openspec-apply-change/references/workflow.md" &&
          resource.name === "references/workflow.md",
      )
    ) {
      throw new Error(
        "无 SDK 的浏览器 Child MCP Resources 列表或 Skill 附属 Resource 元数据不符合预期",
      );
    }
    console.log(
      "✓ 浏览器二级详情：Child MCP 协商后只读取 Tools/Resources 元数据",
    );

    // Catalog 是独立的只读 MCP endpoint；解析不连接、不启动或 provision Child MCP。
    const catalog = await connect(gw.url + "/catalog/mcp");
    try {
      const listed = await requestCatalog(
        catalog.client,
        "mcpp/servers/list",
        {},
        catalogListSchema,
      ...
      if (plan.endpoint.endpointPath !== "/openspec/mcp") {
        throw new Error(
          `Catalog resolve 返回了意外 endpoint '${plan.endpoint.endpointPath}'`,
        );
      }
      ...
      if (!rejectedStaleDigest)
        throw new Error("Catalog 必须拒绝未审阅的 entryDigest");
    ...
    try {
      ...
      const skillRoots = resources.filter((resource) =>
        resource.uri.endsWith("/SKILL.md"),
      );
      if (skillRoots.length !== 12) {
        throw new Error(
          `/openspec/mcp 应投影到 12 个 Skill 根，得到 ${skillRoots.length}`,
        );
      }
      if (
        !resources.some(
        ...
        throw new Error(
          "/openspec/mcp 未投影 openspec-apply-change 的 workflow.md 附属 Resource",
        );
      }
      ...
      const text =
        (doc.contents?.[0] as { text?: string } | undefined)?.text ?? "";
      ...
      const referenceText =
        (reference.contents?.[0] as { text?: string } | undefined)?.text ?? "";
      if (!referenceText.includes("Apply-change workflow reference")) {
        throw new Error("Skill 附属 Resource 内容读取失败");
      }
      console.log(
        `✓ /openspec/mcp：取得 ${skillRoots.length} 个 Skill 根与附属 Resource（SKILL.md/Reference 可读）`,
      );
    } finally {
      await osp.close();
    }
  ...
  try {
    ...
    if (res.status !== 200)
      throw new Error(`纯 handler discover 应 200，得到 ${res.status}`);
    ...
    if (
      ...
      !workerResourcesBody.result?.resources?.some(
        (resource) =>
          resource.uri ===
          "skill://openspec-apply-change/references/workflow.md",
      )
    ...
    console.log(
      "✓ Worker 形态：纯 fetch handler 可发现 Catalog endpoint（无监听进程依赖）",
    );
  } finally {
    await routes.close();
  }
