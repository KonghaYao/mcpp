...
workerGlobal.onmessage = async ({ data }: MessageEvent<any>) => {
  if (data.type === "load") {
    try {
      ...
      if (typeof candidate !== "function")
        throw new Error("Package must export fetch(Request)");
      handler = candidate;
      workerGlobal.postMessage({ type: "ready" });
    } catch (error) {
      workerGlobal.postMessage({
        type: "load-error",
        error: error instanceof Error ? error.message : "WORKER_LOAD_FAILED",
      });
    }
    return;
  }
  ...
  try {
    ...
    if (!(response instanceof Response))
      throw new Error("Worker fetch must return Response");
