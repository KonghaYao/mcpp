...
export function workerRequestHeaders(headers: Headers) {
  ...
  result.forEach((_, name) => {
    if (name.toLowerCase().startsWith("x-internal-")) result.delete(name);
  });
  return result;
}
...
export function publicHeaders(headers: Headers) {
  ...
  headers.forEach((value, name) => {
    if (
      ...
      !name.toLowerCase().startsWith("x-internal-")
    )
...
export function errorStatus(message: string) {
  ...
  if (
    ...
    message === "STDIO_NOT_SUPPORTED" ||
    message === "MISSING_MCP_KEYWORD" ||
    message === "ENTRY_OUTSIDE_PACKAGE"
  )
    return 400;
  return 502;
}
