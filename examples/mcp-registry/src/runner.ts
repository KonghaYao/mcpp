...
export function createRunnerRuntime(
  ...
  const sourceUrl =
    options.sourceUrl ?? process.env.NPM_SOURCE_URL ?? "http://localhost:4873";
  ...
  app.all("/routes/:id/*", async (c) => {
    try {
      ...
      const route =
        c.req.path.replace(`/routes/${c.req.param("id")}`, "") || "/";
      const headers = workerRequestHeaders(c.req.raw.headers);
      const request = new Request(
        new URL(
          route + new URL(c.req.url).search,
          "http://worker.local",
        ).toString(),
  ...
  async function deploy(item: Deployment) {
    ...
    try {
      if (!item.version) {
        ...
        ) as { version: string; integrity?: string };
      ...
      const { entry, server } = validateManifest(
        manifest,
        item.serverId,
        packageRoot,
      );
      ...
      item.description =
        manifest.mcpp?.display?.description ?? manifest.description ?? "";
      item.serverTitle =
        manifest.mcpp?.servers?.[item.serverId]?.title ?? item.serverId;
      ...
      try {
        ...
        const probe = await worker.fetch(
          new Request("http://worker.local/", { method: "GET" }),
        );
      ...
      const message =
        error instanceof Error ? error.message : "DEPLOYMENT_FAILED";
  ...
  for (const item of recovered) {
    deployments.set(item.id, item);
    if (item.status === "queued" && options.autoStart !== false)
      void deploy(item);
  }
...
if (import.meta.main)
  Bun.serve({
    port: Number(process.env.RUNNER_PORT ?? 3001),
    fetch: app.fetch,
  });
export { app, runtime };
