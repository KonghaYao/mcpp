...
function projectPackage(item: PackageRecord): RegistryPackage | undefined {
  ...
  const latest =
    distTags && typeof distTags === "object" && "latest" in distTags
  ...
  const keywords = Array.isArray(item.keywords)
    ? item.keywords.filter((word): word is string => typeof word === "string")
    : [];
  if (!keywords.includes("mcp-plugin")) return undefined;
  const publisher =
    item.publisher &&
    typeof item.publisher === "object" &&
    "username" in item.publisher
      ? String((item.publisher as { username: unknown }).username)
      : undefined;
  const time =
    item.time && typeof item.time === "object" && "modified" in item.time
      ? String((item.time as { modified: unknown }).modified)
      : undefined;
  return {
    name: item.name,
    version,
    ...
    keywords,
    date: typeof item.date === "string" ? item.date : time,
    publisher,
  };
}
...
export function projectSearch(value: unknown): RegistryPackage[] {
  ...
  return objects.flatMap(({ package: item }) =>
    item
      ? [projectPackage(item)].filter((entry): entry is RegistryPackage =>
          Boolean(entry),
        )
      : [],
  );
  ...
  return Object.entries(value as Record<string, unknown>)
    ...
    .flatMap(([, item]) =>
      item && typeof item === "object"
        ? [projectPackage(item as PackageRecord)].filter(
            (entry): entry is RegistryPackage => Boolean(entry),
          )
        : [],
    );
}
...
export class NpmStore {
  private readonly listCache = new Map<
    string,
    { expiresAt: number; value: RegistryPackage[] }
  >();
  ...
  private url(path: string) {
    return new URL(path, this.sourceUrl).toString();
  }
  async list(text = "") {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(search ? "NPM_SEARCH_FAILED" : "NPM_CATALOG_FAILED");
      const value = search
        ? projectSearch(await response.json())
        : projectCatalog(await response.json());
      this.listCache.set(search, {
  ...
  async detail(name: string) {
    ...
    if (cached && cached.expiresAt > Date.now()) return cached.value;
    try {
      const response = await this.fetcher(this.url(encodeURIComponent(name)), {
        signal: AbortSignal.timeout(8_000),
      });
      if (!response.ok)
        throw new Error(
      ...
      const value = {
        ...
        keywords: latest.keywords ?? packument.keywords ?? [],
        readmeHtml: renderReadme(readme),
        mcpp: latest.mcpp,
      };
      this.detailCache.set(name, {
        expiresAt: Date.now() + this.cacheTtlMs,
        value,
      });
      return value;
