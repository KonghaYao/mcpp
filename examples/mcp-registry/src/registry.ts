...
async function runnerDeployments() {
  const response = await fetch(`${runnerUrl}/internal/deployments`, {
...
async function proxyDeployment(c: Context, id: string, suffix: string) {
  ...
  const response = await fetch(
    `${runnerUrl}/routes/${id}${suffix}${new URL(c.req.url).search}`,
    {
      method: c.req.method,
      headers,
      body,
...
app.get("/api/packages", async (c) => {
  try {
    return c.json(await store.list(c.req.query("q")?.slice(0, 100) ?? ""));
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "NPM_SEARCH_FAILED";
    return c.json({ error: message }, errorStatus(message) as 502);
  }
});
app.get("/api/packages/:name{.+}", async (c) => {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "PACKAGE_LOOKUP_FAILED";
    return c.json({ error: message }, errorStatus(message) as 404);
  }
});
app.post("/api/deployments", async (c) => {
  ...
  try {
    ...
    return c.json({ error: message }, errorStatus(message) as 400);
  }
});
  ...
  try {
    ...
    return c.json(
      deployments.map((item) =>
        publicDeployment(item, new URL(c.req.url).origin),
      ),
    );
  } catch {
    return c.json({ error: "RUNNER_UNAVAILABLE" }, 502);
  }
});
...
app.all("/mcp/:id/*", async (c) => {
  try {
    ...
    if (!/^[0-9a-f-]{36}$/.test(id))
      return c.json({ error: "INVALID_DEPLOYMENT_ID" }, 400);
...
if (import.meta.main)
  Bun.serve({
    port: Number(process.env.REGISTRY_PORT ?? 3000),
    fetch: app.fetch,
  });
export { app };
