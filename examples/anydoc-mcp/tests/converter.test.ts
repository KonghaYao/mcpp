...
describe("local file guard", () => {
    ...
    test("rejects URLs and unsupported formats", async () => {
        const { root } = await fixture();
        await expect(resolveSafeFile("https://example.invalid/file.pdf", [root])).rejects.toThrow("URLs are not supported");
        const text = join(root, "notes.txt");
        await writeFile(text, "nope");
        await expect(resolveSafeFile(text, [root])).rejects.toThrow("Unsupported document format");
    });
    ...
    test("rejects containment escapes and symlinks", async () => {
        ...
        await expect(resolveSafeFile(link, [root])).rejects.toThrow("Symbolic links");
    });
    ...
    test("rejects non-regular files", async () => {
        ...
        await expect(resolveSafeFile(directoryWithExtension, [root])).rejects.toThrow("regular file");
    });
});
