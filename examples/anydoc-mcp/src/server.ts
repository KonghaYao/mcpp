...
export function createAnydocServer(): McpServer {
    const server = new McpServer(
        ...
        { instructions: "Converts guarded local document files to Markdown. URLs are not supported." },
    );
    ...
    server.registerTool(
        "convert_document",
        {
            ...
            description: "Convert a local document under ANYDOC_ALLOWED_ROOTS to Markdown. URLs are rejected.",
            inputSchema: z.object({ path: z.string().min(1).describe("Absolute local file path") }),
        },
        async ({ path }) => {
            try {
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: error instanceof Error ? error.message : "Document conversion failed" }],
                };
            }
        },
    );
